        ...
        {	
    		...
    		if($id = $this->branch_model->addModel($data))
    		{
				...
				$log_data = array(
						...
						'table_id' => $id,
						'message'  => $branch_name.' is added successfully.'
					);
				$this->log_model->insert_log($log_data);

				$this->session->set_flashdata('success', $branch_name.' is added successfully.');
				redirect('branch');
			}
			else
			{
				$this->session->set_flashdata('failure', $branch_name.' is failed to added.');
        ...
        else
        {
			...
			if($this->branch_model->editModel($data,$id)){
				$log_data = array(
						'user_id'  => $this->session->userdata('user_id'),
						'table_id' => $id,
						'message'  => $branch_name.' is updated successfully.'
					);
				$this->log_model->insert_log($log_data);

				$this->session->set_flashdata('success', $branch_name.' is updated successfully.');
				redirect('branch');
			}
			else{
				$this->session->set_flashdata('failure', $branch_name.' is failed to update.');
				redirect("branch",'refresh');
			}
		}
	}
        ...
        else
        {
        	...
        	if($this->branch_model->deleteModel($id))
        	{
				$log_data = array(
						'user_id'  => $this->session->userdata('user_id'),
						'table_id' => $id,
						'message'  => $branch->branch_name.' is deleted successfully.'
					);
				$this->log_model->insert_log($log_data);

				$this->session->set_flashdata('success', $branch->branch_name.' is deleted successfully.');
				redirect('branch');
			}
			else
			{
				$this->session->set_flashdata('failure', $branch->branch_name.' is failed to add.');
				redirect("branch",'refresh');
			}	
        }
