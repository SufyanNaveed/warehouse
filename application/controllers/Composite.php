	...
	public function add(){ 
		...
		if($_POST) {
			...
			if($id = $this->composite_model->addModel($composite)){
				...
				$this->session->set_flashdata('success', $composite_product_name.' is successfully saved');
				redirect('composite');
			}else{
				fail:
				$this->session->set_flashdata('failure', $composite_product_name.' is failed to save.');
				redirect('composite');
			}
        }else{
