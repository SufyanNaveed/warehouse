			...
			if($this->input->post('type')=="R")
			{
				...
				if($id = $this->receipt_model->addReceipt($data)){
					$log_data = array(
							...
							'table_id' => $id,
							'message'  => $type.' is added successfully.'
						);
					...
					$this->session->set_flashdata('success', $type.' is added successfully.');
					redirect('receipt','refresh');
				}
				else
				{
					$this->session->set_flashdata('failure', $type.' is failed added.');
					redirect("receipt",'refresh');
				}
			}
			else
	...
	public function editCategory(){
        ...
        else
        {
			...
			if($this->category_model->editModel($data,$id)){
				...
				$this->session->set_flashdata('failure', $category_name.' is failed to updated.');
				redirect("category",'refresh');
			}
		}
	}
