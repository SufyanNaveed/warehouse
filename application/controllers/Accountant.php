	...
	public function addAccountant(){ 
		...
		else
		{
			...
			if($id = $this->accountant_model->addUsers($users)){
				...
				$this->session->set_flashdata('success', $first_name.' is Successfully Inserted.'); 
				redirect('biller','refresh');
			}
			else{
				$this->session->set_flashdata('failure', $first_name.' is not created.');				
				redirect("biller",'refresh');
			}
		}
	}
		...
		else
		{
			...
			else
			{
				...
				if($this->accountant_model->editUser($users,$id)){
					...
					$this->session->set_flashdata('failure', $first_name.'  is not updated.');
					redirect("biller",'refresh');
				}			
			}			
