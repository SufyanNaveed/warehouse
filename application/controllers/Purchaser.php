			...
			if($id = $this->biller_model->addUsers($users)){
				...
				if($id = $this->assign_warehouse_model->assignWarehouse($assign_warehouse_new)){
					$log_data = array(
							...
							'table_id' => $id,
							'message'  => $first_name.' is Successfully Created.'
						);
					$this->log_model->insert_log($log_data);
					//redirect('assign_warehouse','refresh');
				}
				$this->session->set_flashdata('success', $first_name.' is Successfully Created.'); 
				redirect('biller','refresh');
			}
			else{
				$this->session->set_flashdata('failure', $first_name' is failed to created.');				
				redirect("biller",'refresh');
			}
		}
	}
