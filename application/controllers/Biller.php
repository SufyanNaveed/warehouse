				...
				$data 		= array(
					...
					if(($biller_id = $this->biller_model->addModel($data)) && $warehouse_assignment_id = $this->assign_warehouse_model->assignWarehouse($warehouse_management))
					{
						$log_data = array(
								...
								'table_id' => $biller_id,
								'message'  => $biller_name.' is added successfully.'
							);
						$this->log_model->insert_log($log_data);
						
						$this->session->set_flashdata('success', $biller_name.' is added successfully.'); 
						redirect('biller','refresh');
					}
					else{
						$this->session->set_flashdata('failure', $biller_name.' is failed to add.');				
						redirect("biller",'refresh');
					}
			}
			// else{
