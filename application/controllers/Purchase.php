...
class Purchase extends MY_Controller
{
	...
	*/
	...
	public function purchaseItemReceive($purchase_id, $purchase_item_id, $product_id, $quantity, $warehouse_id, $batch){
		...
		$purchase_item 			= $this->purchase_model->getPurchaseItem($purchase_item_id);
		...
		$this->purchase_model->addProductInWarehouse($product_id,$quantity,$warehouse_id,$warehouse_data,$purchase_item->expiry,$purchase_item->price);
		...
		else
		{
			...
			if($this->purchase_model->editPurchaseRecord($id,$data)){
				...
				if($js_data!=null){
					...
					if($this->purchase_model->deletePurchaseItemsByPurchaseId($id))
					{
						foreach ($js_data as $key => $value) 
						{
							$product_id = $value->product_id;
							...
							$this->purchase_model->addPurchaseItem($data);
