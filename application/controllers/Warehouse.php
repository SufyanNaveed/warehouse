        ...
        else
        {
			...
			if($id = $this->warehouse_model->addModel($data)){ 
				$log_data = array(
						'user_id'  => $this->session->userdata('user_id'),
						'table_id' => $id,
						'message'  => $warehouse_name.' is Inserted.'
					);
				$this->log_model->insert_log($log_data);

				$this->session->set_flashdata('success', $warehouse_name.' is Inserted.');
				redirect('warehouse','refresh');
			}
			else{
				$this->session->set_flashdata('failure', $warehouse_name.' is not be Inserted.');
				redirect("warehouse",'refresh');
			}
		}
        ...
        else
        {
			...
			if($this->warehouse_model->editModel($data,$id)){
				$log_data = array(
						'user_id'  => $this->session->userdata('user_id'),
						'table_id' => $id,
						'message'  => $warehouse_name.' is updated.'
					);
				$this->log_model->insert_log($log_data);

				$this->session->set_flashdata('success', $warehouse_name.' is updated.');
				redirect('warehouse');
			}
			else{
				$this->session->set_flashdata('failure', $warehouse_name.' can not be Updated.');
				redirect("warehouse",'refresh');
			}
		}
	}
	...
	public function delete($id)
	{
		...
		else
		{
			...
			if($this->warehouse_model->isPrimaryWarehouse($id)){
				...
				$this->session->set_flashdata('failure', 'Primary warehouse can not be Deleted.');
				redirect("warehouse",'refresh');
				...
				if($this->warehouse_model->deleteModel($id)){
					$log_data = array(
						...
						$this->log_model->insert_log($log_data);
						$this->session->set_flashdata('success', $warehouse->warehouse_name.' is successfully deleted');
					redirect('warehouse','refresh');
				}
				else{
					$this->session->set_flashdata('failure', $warehouse->warehouse_name.' can not be Deleted.');
					redirect("warehouse",'refresh');
				}
			}
		}
