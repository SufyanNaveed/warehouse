                      ...
                      <select class="form-control select2" id="po_status" name="po_status" style="width: 100%;">
                        <option value="pending" 
                          <?php 
                            ...
                            if($data[0]->po_status == "pending")
                              echo ' selected';
                          ?>
                        ><?php echo "Pending"; ?></option>
                        <option value="approved"
                          <?php 
                            ...
                            if($data[0]->po_status == "approved")
                              echo ' selected';
                          ?>
                        ><?php echo "Transfer to Purchase Entry"; ?></option>
                        <option value="withdraw"
                          <?php 
                            if(isset($po_status)) 
                            {
                              if($po_status == "withdraw")
                                echo ' selected';
                            }

                            if($data[0]->po_status == "withdraw")
                              echo ' selected';
                          ?>
                        ><?php echo "Withdraw"; ?></option>
                      </select>
                      ...
                      <tbody>
                        <?php
                          ...
                          <tr>
                            ...
                            <td>
                              <input type="text" class="form-control" value="<?php echo $key->batch ?>" data-rule="batch" name='batch' id='batch'>
                            </td>
                            <td>
                              <input type="text" class="form-control datepicker" value="<?php echo $key->expiry ?>" data-rule="expiry" name='expiry' id='expiry'>
                            </td>
                            <td>
                          ...
                          <?php
                            ...
                            // $w_product = json_encode($w_product_data);
...
$(document).ready(function(){
    ...
    existing_w_product_id = JSON.parse($("#table_data").val());
    // existing_w_product_id = JSON.parse($("#table_data2").val());
    ...
    $("table.product_table").on("change", 'input[name^="price"], input[name^="qty"],input[name^="igst"],input[name^="cgst"],input[name^="sgst"],input[name^="tax_type"],select[name^="item_brand"],input[name^="batch"],input[name^="expiry"]', function (event) {
    ...
    $("#submit").click(function(event){
      ...
      $("table.product_table tr").each(function (index, value){
        $(this).find('input[name^="qty"]').trigger("change");
      });
      ...
      //     table_data_array[i] = existing_w_product_id[i];
      //   }
      // }
