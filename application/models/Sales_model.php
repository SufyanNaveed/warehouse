...
class Sales_model extends CI_Model
{
	...
	*/
	public function getProduct($product_id,$warehouse_id){
		return $this->db->select('p.product_id,p.code,p.hsn_sac_code,p.unit,p.name,p.cost,p.price,p.alert_quantity,p.image,p.category_id,p.subcategory_id,p.tax_id,wp.quantity,wp.warehouse_id')
			 ->from('products p')
	...
	*/
	...
	public function getBarcodeProducts($term,$warehouse_id){
			$where = "(p.code LIKE '%".$term."%' OR p.name LIKE '%".$term."%')";
		   	 $this->db->select('p.product_id,p.code,p.name,wp.batch,wp.id as wp_id,wp.quantity as wp_quantity')
					 	 ->from('products p')
						 ->join('warehouses_products wp','p.product_id = wp.product_id')
						 ->where('wp.warehouse_id',$warehouse_id)
						 ->where('wp.quantity > 0')
	...
	*/
	public function getProductUseCode($product_id,$warehouse_id){
		return $this->db->select('p.*,wp.quantity,wp.warehouse_id,wp.id as wp_id,wp.expiry,wp.batch,wp.price')
			 ->from('products p')
			 ->join('warehouses_products wp','p.product_id = wp.product_id')
			 ->where('wp.warehouse_id',$warehouse_id)
