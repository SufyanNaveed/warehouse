...
class Ion_auth_model extends CI_Model
{
	...
	**/
	...
	public function allReports(){
		...
		/*  Purchase */
		$data['todayPurchase'] = $this->db->select('sum(pi.quantity) as item, sum(wp.cost*pi.quantity) as value')
								  ->from('purchases p')
		...
		$data['weekPurchase'] = $this->db->select('sum(pi.quantity) as item, sum(wp.cost*pi.quantity) as value')
								  ->from('purchases p')
								  ->join('purchase_items pi','p.purchase_id = pi.purchase_id')
								  ->join('products pr','pr.product_id = pi.product_id')
								  ->join('warehouses_products wp','wp.product_id = pr.product_id')
		...
		$data['monthPurchase'] = $this->db->select('sum(pi.quantity) as item, sum(wp.cost*pi.quantity) as value')
								  ->from('purchases p')
								  ->join('purchase_items pi','p.purchase_id = pi.purchase_id')
								  ->join('products pr','pr.product_id = pi.product_id')
								  ->join('warehouses_products wp','wp.product_id = pr.product_id')
								  ...
								  ->get()->result();
		$data['yearPurchase'] = $this->db->select('sum(pi.quantity) as item, sum(wp.cost*pi.quantity) as value')
								  ->from('purchases p')
								  ->join('purchase_items pi','p.purchase_id = pi.purchase_id')
								  ->join('products pr','pr.product_id = pi.product_id')
								  ->join('warehouses_products wp','wp.product_id = pr.product_id')
								  ...
								  ->like('p.date',date('Y'))
								  ->get()->result();
		$data['allPurchase'] = $this->db->select('sum(pi.quantity) as item, sum(wp.cost*pi.quantity) as value')
								  ->from('purchases p')
								  ->join('purchase_items pi','p.purchase_id = pi.purchase_id')
								  ->join('products pr','pr.product_id = pi.product_id')
								  ->join('warehouses_products wp','wp.product_id = pr.product_id')
		...
		/* Sales */
		$data['todaySales'] = $this->db->select('sum(si.quantity) as item,sum(wp.price*si.quantity) as value')
								  ->from('sales s')
								  ...
								  ->get()->result();
		$data['weekSales'] = $this->db->select('sum(si.quantity) as item,sum(wp.price*si.quantity) as value')
								  ->from('sales s')
								  ->join('sales_items si','s.sales_id = si.sales_id')
								  ->join('products pr','pr.product_id = si.product_id')
								  ->join('warehouses_products wp','wp.product_id = pr.product_id')
		...
		$data['monthSales'] = $this->db->select('sum(si.quantity) as item,sum(wp.price*si.quantity) as value')
								  ->from('sales s')
								  ->join('sales_items si','s.sales_id = si.sales_id')
								  ->join('products pr','pr.product_id = si.product_id')
								  ->join('warehouses_products wp','wp.product_id = pr.product_id')
								  ...
								  ->get()->result();
		$data['yearSales'] = $this->db->select('sum(si.quantity) as item,sum(wp.price*si.quantity) as value')
								  ->from('sales s')
								  ->join('sales_items si','s.sales_id = si.sales_id')
								  ->join('products pr','pr.product_id = si.product_id')
								  ->join('warehouses_products wp','wp.product_id = pr.product_id')
								  ...
								  ->like('s.date',date('Y'))
								  ->get()->result();
		$data['allSales'] = $this->db->select('sum(si.quantity) as item,sum(wp.price*si.quantity) as value')
								  ->from('sales s')
								  ->join('sales_items si','s.sales_id = si.sales_id')
								  ->join('products pr','pr.product_id = si.product_id')
								  ->join('warehouses_products wp','wp.product_id = pr.product_id')
		...
		$data['product'] = $this->db->select('p.product_id')
									...
									->get()
									->result();
		$data['warehouse_product'] = $this->db->select('sum(wp.quantity) as warehouse_item,sum(wp.quantity*wp.cost) as value')
											 ->from('warehouse w')
											 ->join('warehouses_products wp','w.warehouse_id = wp.warehouse_id')
											 ->join('products p','wp.product_id = p.product_id')
											 // ->where('p.user_id',$this->session->userdata('user_id'))
											 ...
											 ->get()
											 ->result();
		$data['total_sales'] = $this->db->select('sum(si.quantity*wp.price) as total_sales')
										->from('sales s')
										->join('sales_items si','si.sales_id = s.sales_id')
		...
		$data['supplier'] = $this->db->query('SELECT count(*) as count FROM users u JOIN users_groups ug ON u.id = ug.user_id JOIN groups g ON g.id = ug.group_id WHERE g.name = "supplier"')->row();
