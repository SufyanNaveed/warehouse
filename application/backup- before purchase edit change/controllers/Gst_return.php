...
class Gst_return extends MY_Controller
{
	...
	public function index(){
		if(!$this->user_model->has_permission("gstr1"))
        {
            $this->load->view('layout/restricted'); 
        }
        else
        {
        	$this->load->view('gst_return/gstr');	
        }
	}
	/*
		get date and month
	*/
	...
	public function gstr2(){
		if(!$this->user_model->has_permission("gstr2"))
        {
            $this->load->view('layout/restricted'); 
        }
        else
	...
	*/
	public function GSTR1b2b($month,$year){
		...
		$delimiter = ",";
        $newline = "\r\n";
        $filename = "B2B-".date('d-m-Y H:i:s').".csv";
        // $filename = "b2b.csv";
        $data = $this->dbutil->csv_from_result($data, $delimiter, $newline);
        force_download($filename, $data);
	}
	...
	*/
	public function GSTR1b2cl($month,$year){
		...
		$delimiter = ",";
        $newline = "\r\n";
        $filename = "B2C-".date('d-m-Y H:i:s').".csv";
        $data = $this->dbutil->csv_from_result($data, $delimiter, $newline);
        force_download($filename, $data);
	}
	/*
