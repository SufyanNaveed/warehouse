	...
	*/
	public function addProductInWarehouse($product_id,$quantity,$warehouse_id,$warehouse_data){
		...
		if($query->num_rows()>0){
			...
			$purchase_item_sql = 'select expiry from purchase_item where product_id = ? AND warehouse_id = ? AND batch = ?';
			$query1 = $this->db->query($purchase_item_sql,array($product_id,$warehouse_id,$warehouse_data['batch']));
			$expiry = $query1->row()->batch;
			
			$warehouse_data['expiry'] = $expiry;

			$sql = "insert into warehouses_products (product_id,warehouse_id,quantity,batch,expiry) values (?,?,?,?,?)";
